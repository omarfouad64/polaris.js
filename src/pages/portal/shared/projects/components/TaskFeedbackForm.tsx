...
interface TaskFeedbackFormProps {
    ...
    feedbackId?: string
    addTaskFeedback: (taskId: string, instructorId: string, instructorName: string, comment: string) => any
    editTaskFeedback: (feedbackId: string, newComment: string) => void
}

/**
 * TaskFeedbackForm – modal for instructors to add or edit feedback on project tasks.
                ...
                <div className="border-b border-surface-container-high p-6 flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-jakarta font-bold text-on-surface">
                            {feedbackId ? 'Edit Task Feedback' : 'Add Task Feedback'}
                        </h2>
                    ...
                    {!submitted ? (
                        <>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-4 py-2 rounded-lg font-jakarta font-semibold transition-colors ${feedbackText.trim() && !isSubmitting
                                            ? 'bg-primary text-on-primary hover:bg-primary-container'
                                            : 'bg-surface-container text-on-surface-variant cursor-not-allowed'
                                        }`}
                                >
                                    {isSubmitting ? 'Submitting...' : feedbackId ? 'Update Feedback' : 'Submit Feedback'}
                                </button>
                            </div>
                        </>
                    ) : (
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="font-jakarta font-bold text-on-surface mb-2">
                                Feedback {feedbackId ? 'Updated' : 'Submitted'}!
                            </p>
                            <p className="text-body-sm text-on-surface-variant">
                                Your feedback has been {feedbackId ? 'updated' : 'added'} to the task. A notification will be sent to the team.
                            </p>
                        </div>
                    )}
                </div>
