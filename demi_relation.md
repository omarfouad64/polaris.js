# Minimal Dummy Data Relations

This document defines the absolute minimum, consistent dummy data required to showcase every feature of the Polaris.js platform. By loading this relational graph into your hooks, you can test all user flows—from project creation and instructor feedback to moderation and internship applications.

## The Relational Graph Overview

We will simulate a system with four main actors:
1. **Alice (Student)**: `student_1`
2. **Dr. Bob (Instructor)**: `instructor_1`
3. **TechCorp (Employer)**: `employer_1`
4. **Admin**: `admin_1`

The central scenario: Alice has multiple projects across Dr. Bob's courses, receives task-specific and project-level feedback, interacts with the moderation system, messages employers, applies to active internships, and displays past completed internships.

---

### 1. Courses & Instructors

Dr. Bob teaches two courses.
...
### 2. Student Portfolio & History

Alice, the student taking the courses, with past internship experience.
...
// CompletedInternship (Past experience)
{
  "id": "completed_int_1",
...
Alice creates two projects. She invites another student (Charlie, implicitly) to one of them.

```json
// Implicit Project Records
[
  {
    "id": "project_1",
...
// ProjectCollaborator (Alice is owner for project_1)
{
...
// ProjectInvitation (Alice invites Charlie to project_1)
{
  "id": "invite_1",
  "projectId": "project_1",
...
### 4. Instructor Feedback & Rating

Dr. Bob provides both task-level and project-level feedback.

```json
// TaskFeedback (Specific task in project_2)
{
  "id": "task_feedback_1",
  "taskId": "task_101",
  "instructorId": "instructor_1",
...
### 5. Moderation (Flags & Appeals)

The admin flags Alice's second project.
...
### 6. Employer & Internships

TechCorp posts multiple internships and tracks performance metrics.
...
// InternshipApplication (Alice applies to Frontend role)
{
  "id": "application_1",
...
Alice bookmarks projects, messages TechCorp, and receives notifications.
...
// Messages between Alice and TechCorp
[
  {
    "id": "msg_1",
    "senderId": "student_1",
...
### 8. Per-User Notification Feeds

To ensure every notification type is covered (`message`, `internship_status`, `project_invitation`, `feedback`, `flag`, `admin`, `appeal_response`), here are the localized notification feeds for each user in our system.

```json
// 1. Student Notifications (Alice - student_1)
[
  {
    "id": "notif_s1",
    "type": "feedback",
...
// 2. Instructor Notifications (Dr. Bob - instructor_1)
[
  {
    "id": "notif_i1",
    "type": "project_invitation",
...
// 3. Employer Notifications (TechCorp - employer_1)
[
  {
    "id": "notif_e1",
    "type": "admin",
...
// 4. Admin Notifications (System Admin - admin_1)
[
  {
    "id": "notif_a1",
    "type": "flag",
